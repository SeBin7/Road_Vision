import os
os.environ['OPENCV_LOG_LEVEL'] = 'OFF'   # 또는 'SILENT'
...
class VideoInference:
    def __init__(self, seq_len=SEQ_LEN):
        ...
        self.cnn.load_state_dict(torch.load(
            './best_cnn_feature_extractor.pth', map_location=device))
        self.cls.load_state_dict(torch.load(
            './best_gru_mlp_classifier.pth', map_location=device))
    ...
    def predict(self):
        if len(self.buffer) < self.seq_len:
            return None, None
        x = torch.stack(list(self.buffer)).unsqueeze(0)
        b,t,c,h,w = x.size()
        with torch.no_grad():
            feat = self.cnn(x.view(b*t,c,h,w).to(device),
                            flatten=True).view(b,t,-1)
            logit = self.cls(feat)
    ...
    while True:
        if not paused:
            ...
            display_frame = cv2.resize(
                frame, (int(w*disp_scale), int(h*disp_scale)))

            font_scale, thickness, y_pred, y_time = \
                get_text_params(display_frame.shape[0])

            if label:
                txt = f'{label}: {conf:.1f}%'
                cv2.putText(display_frame, txt,
                            (int(disp_scale*h*0.03), y_pred),
                            FONT, font_scale, BAR_COLOR,
                            thickness, cv2.LINE_AA)

            info = (f'{frames_to_time(current,fps)}/'
                    f'{frames_to_time(total,fps)}  '
                    f'[{frames_to_time(start_frame,fps)} - '
                    f'{frames_to_time(end_frame,fps)}]')
            cv2.putText(display_frame, info,
                        (int(disp_scale*h*0.03), y_time),
                        FONT, font_scale*0.7,
        ...
        if   key == ord(' '): paused = not paused
            ...
            paused = False
            print(f'▶ 구간 재추론: {frames_to_time(start_frame,fps)} ~ '
                  f'{frames_to_time(end_frame,fps)}')
        elif key == ord('q'):
            break
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Road-Vision timeline inference')
    parser.add_argument('video',
                        help='입력 동영상 파일(.mp4, .avi …)')
