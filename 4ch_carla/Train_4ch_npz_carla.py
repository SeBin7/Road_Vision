...
# 2) torch 임포트 후, xpu.has_2d_block_array stub 설정
import torch
...
from Dataset_4ch_npz import NPZSequenceBuilder, NPZWindowedDataset
...
import os
os.environ["IPEX_ENABLE_CHANNEL_LAST"] = "false"
...
# NPZ 디렉토리 확인
npz_dir = "/home/kyj28/workspace/Road_Vision/Road_Vision_video/Road_Vision/4ch_carla/npz_sequences"  # 실제 경로로 변경
...
# ──────────────────────────────────────────
def train():
    video_root = '/home/kyj28/workspace/Road_Vision/Road_Vision_video/Road_Vision/Train_3ch_Carla'
    npz_dir = '/home/kyj28/workspace/Road_Vision/Road_Vision_video/Road_Vision/4ch_carla/npz_sequences'  # NPZ 파일이 저장될 디렉토리
    calibration_dir = './hailo_calibration_4ch_carla'
    ...
    label_map = check_npz_labels(npz_dir)        # class_map = {'A':0,'B':1}
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(cnn.state_dict(), "cnn_feature_extractor_4ch_carla.pth")
            torch.save(cls.state_dict(), "gru_mlp_classifier_4ch_carla.pth")
            torch.save(cnn, "cnn_feature_extractor_4ch_carla.pt")
            torch.save(cls, "gru_mlp_classifier_4ch_carla.pt")

            # ONNX export
            dummy = torch.randn(1,4,224,224).to(device)
            torch.onnx.export(cnn, dummy,
                              "cnn_feature_extractor_4ch_carla.onnx",
