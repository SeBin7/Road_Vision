...
def convert_calibration_images_to_npy(image_dir, output_path, target_size=(224, 224)):
    """
    캘리브레이션 이미지 폴더를 .npy 파일로 변환 (Hailo 형식에 맞게)
    ...
    # ⚠️ ImageNet 정규화 제거 - Hailo는 원본 이미지 사용
    transform = transforms.Compose([
        transforms.Resize(target_size),
        transforms.ToTensor(),  # [0,1] 범위로 변환 및 HWC→CHW
        # transforms.Normalize() 제거!
    ])
    ...
    for i, img_path in enumerate(image_paths):
        try:
            ...
            # 전처리 적용 (정규화 제외)
            tensor = transform(image)  # Shape: (C, H, W), 범위: [0, 1]
            
            # 🔄 CHW → HWC 변환
            tensor_hwc = tensor.permute(1, 2, 0)  # (C, H, W) → (H, W, C)
            
            # 🔄 [0, 1] → [0, 255] 변환 (Hailo는 uint8 형태 선호)
            tensor_255 = (tensor_hwc * 255).numpy().astype(np.uint8)
            
            all_images.append(tensor_255)
            total_count += 1
    ...
    # 🔄 모든 이미지를 하나의 numpy 배열로 스택 (N, H, W, C)
    ...
    print(f"✅ 변환 완료!")
    print(f"📊 최종 Shape: {calibration_data.shape}")  # (N, H, W, C)
    print(f"📊 데이터 타입: {calibration_data.dtype}")  # uint8
    print(f"📊 값 범위: [{calibration_data.min()}, {calibration_data.max()}]")  # [0, 255]
